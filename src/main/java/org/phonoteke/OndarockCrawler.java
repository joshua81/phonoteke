...
//				DBObject articleDB = articles.findOne(getId());
//				if(articleDB != null)
//				{
//					return;
//				}
...
//				articles.insert(json);
				logger.info("Document " + getURL(url) + " added");
			} 
			catch (Throwable t) 
			{
	...
	private class Article
	{
		...
		public Article(Document doc, String url)
		{
			id = initId();
			type = initType(url);
			content = initContent(doc);
			band = initBand(doc);
			album = initAlbum(doc);
			cover = initCover(doc);
			author = initAuthor(doc);
			genre = initGenre(doc);
			year = initYear(doc);
			label = initLabel(doc);
			vote = initVote(doc);
			milestone = initMilestone(url);
			creationDate = initCreationDate(doc);
		}
		
		protected String initContent(Document doc) {
			try
			{
				// remove all images
