...
public class OndarockCrawler extends AbstractCrawler
{
	...
	@Override
	protected String getDocumentBand(String url, Document doc) {
	...
	@Override
	protected String getDocumentAlbum(String url, Document doc) {
		Element intestazioneElement = null;
	...
	@Override
	protected Date getDocumentCreationDate(String url, Document doc) {
		try
		{
			...
			case REVIEW:
				...
				if(reviewDate == null)
				{
					java.util.Date date = new SimpleDateFormat("dd/MM/yyyy").parse("01/01/" + getDocumentYear(url, doc));
					reviewDate = new Date(date.getTime());
				}
	...
	@Override
	protected String getDocumentCover(String url, Document doc) {
		try
		{
	...
	@Override
	protected String getDocumentAuthor(String url, Document doc) {
		String author = null;
	...
	@Override
	protected String getDocumentGenre(String url, Document doc) {
		switch (getDocumentType(url)) {
		case REVIEW:
	...
	@Override
	protected Integer getDocumentYear(String url, Document doc) {
		switch (getDocumentType(url)) {
		case REVIEW:
			Element datiElement = doc.select("div[id=dati]").first();
			String dati = datiElement.text();
	...
	@Override
	protected String getDocumentLabel(String url, Document doc) {
		switch (getDocumentType(url)) {
		case REVIEW:
			Element datiElement = doc.select("div[id=dati]").first();
			String dati = datiElement.text();
	...
	@Override
	protected Float getDocumentVote(String url, Document doc) {
		try
		{
			switch (getDocumentType(url)) {
			case REVIEW:
	...
	@Override
	protected Boolean getDocumentMilestone(String url, Document doc) {
		switch (getDocumentType(url)) {
		case REVIEW:
	...
	@Override
	protected AbstractCrawler.TYPE getDocumentType(String url) {
	...
	protected Boolean shouldVisit(String url) {
		return getDocumentType(url) != null;
	}
}